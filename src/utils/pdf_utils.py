...
from PIL import Image  # <-- ADDED: Required to adjust Pillow's safety limits
...
# SAFETY OVERRIDE: Allow Pillow to load massive images so our downscaling logic can handle them.
# The default limit is 178,956,970 pixels. We disable the strict limit because the code below
# immediately downscales any image exceeding MAX_SAFE_DIMENSION, preventing memory exhaustion.
...
def pdf_to_images(
    ...
    try:
        logger.debug(
            f"Converting PDF to images: {pdf_path_obj.name} (DPI={dpi}, max_pages={max_pages})"
        )
        ...
        for img in pil_images:
            ...
            if max(h, w) > MAX_SAFE_DIMENSION:
                ...
                logger.info(
                    f"Downscaled massive image from {w}x{h} to {new_size[0]}x{new_size[1]}"
                )  # Changed to info so you can see it happening
