...
# =============================================================================
# DEBUG VISUALIZATION & ARTIFACT GENERATION
# =============================================================================
...
class DebugVisualizer:
    ...
    def save_yolo_debug(
        self, image_stem: str, image: np.ndarray, detections: list
    ) -> None:
    ...
    def save_title_block(
        self, image_stem: str, crop_type: str, crop: np.ndarray
    ) -> None:
        folder = self._get_folder("macro_vision")
        if not folder:
            return
    ...
    def save_debug_roi(
        self, image_stem: str, roi: np.ndarray, idx: int, label: str
    ) -> None:
        """Saves Macro-regions like the Bottom-Right Quadrant or Anchor Search Zones."""
        folder = self._get_folder("macro_vision")
        if not folder or roi is None or roi.size == 0:
            return
    ...
    def save_ocr_debug(
        ...
        if not folder or roi_img is None or roi_img.size == 0:
            return
        ...
        try:
            ...
            draw.text(
                ...
                f"OCR: {display_text}",
                font=title_font,
                fill=(0, 100, 0) if text else (200, 0, 0),
            )
