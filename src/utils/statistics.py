...
def generate_pipeline_run_data(
    ...
    # ✅ STEP 1: Implement Tri-State Classification (Success, HITL, Failure)
    successful_extractions = [r for r in results if r.get("status") == "Success"]
    hitl_extractions = [
        r for r in results if r.get("status") == "Manual Review Required"
    ]
    failed_extractions = [
        r
        for r in results
        if r.get("status") not in ["Success", "Manual Review Required"]
    ]

    success_count = len(successful_extractions)
    hitl_count = len(hitl_extractions)
    failure_count = len(failed_extractions)

    success_rate = (success_count / total_files) * 100 if total_files > 0 else 0.0
    hitl_rate = (hitl_count / total_files) * 100 if total_files > 0 else 0.0
    ...
    parallel_speedup = (
        (total_processing_time / actual_wall_time) if actual_wall_time > 0 else 1.0
    )
    ...
    for res in results:
        ...
        corrected_job = _safe_get(res, "corrected_job") or _safe_get(
            res, "job_number", ""
        )
        confidence = _safe_get(res, "confidence") or _extract_confidence(
            _safe_get(res, "method", "")
        )
        reason = _safe_get(res, "reason") or _extract_correction_reason(
            _safe_get(res, "method", "")
        )
        method = _safe_get(res, "ocr_method") or _extract_ocr_method(
            _safe_get(res, "method", "")
        )
        ...
        file_records.append(
            {
                ...
                "processing_time_sec": float(
                    _safe_get(res, "processing_time_sec", 0.0)
                ),
                "method": str(method),
                "error": (
                    str(_safe_get(res, "error", ""))
                    if _safe_get(res, "error")
                    else None
                ),
            }
        )
    ...
    pipeline_run_data = {
        "batch_metadata": {
            ...
            "successful_autofiling": success_count,
            "manual_review_required": hitl_count,
    ...
    # Calculate metrics for both success and HITL (since both did work!)
    for r in successful_extractions + hitl_extractions:
    ...
    lines: List[str] = [
        ...
        f"Successful Auto-Filing: {success_count} ({success_rate:.1f}%)",
        f"Manual Review (HITL):   {hitl_count} ({hitl_rate:.1f}%)",
        f"Hard Failures:          {failure_count}",
    ...
    for method in sorted(
        method_times.keys(), key=lambda m: sum(method_times[m]), reverse=True
    ):
    ...
    lines.append("\n==================================================")
    lines.append(f"{'FILENAME': <25} | {'AUTO-FILED JOB': <20} | {'TIME': <12} | ")
    lines.append("--------------------------------------------------")
    ...
    for r in sorted(successful_extractions, key=lambda x: x.get("filename", " ")):
        ...
        time_sec = float(r.get("processing_time_sec", 0.0))
        lines.append(
            f"🟢 {fname: <23} | {job_num: <20} | {format_duration(time_sec): <12} | "
        )

    # ✅ STEP 2: Dedicated queue for Manual Review (HITL) candidates
    if hitl_extractions:
        ...
        for r in sorted(hitl_extractions, key=lambda x: x.get("filename", " ")):
            ...
            lines.append(
                f"• {fname: <21} | System Guess: {guess: <15} | Reason: {error}"
            )
